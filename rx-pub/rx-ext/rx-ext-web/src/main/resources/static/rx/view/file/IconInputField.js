 ...
 * 
 * */
Ext.define('Rx.view.file.IconInputField', {
    ...
    cls:'r-iconinputfield',
    
	setValue:function(id){
		this.lookupI('field').setValue(id);
	},
	getValue:function(){
		return this.lookupI('field').getValue();
	},
    	...
    	Ext.apply(me,{
    		...
    		items:[{
            	...
            	items:[{
            		...
            		xtype:'image',
            		autoEl:{tag:'a',href:href,target:'_blank',style:'border:1px dashed #d0d0d0;margin-right:5px;width:80px;height:80px'},
            		itemId:'_img_',
            		width:80,
    		        height:80,
    		        src:src,
            		...
            		transformRawValue:function(v){
            			var fi = this.up('iconinput');
            			var imgf = fi.lookupI('_img_');
            			if(v && v != ""){
            				FileController.getPath({
            					params:{path:v,width:80,height:80},
            					callback:function(o,f,r){
            						if(f){
            							var imgff = this.lookupI('_img_');
